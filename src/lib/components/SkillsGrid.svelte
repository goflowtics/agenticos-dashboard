<script lang="ts">
	...
	const dispatch = createEventDispatcher<{
		run: { skill: string; domain: string; input?: string };
	}>();

	let inputs = $state<Record<string, string>>({});

	function run(skill: string, domain: string) {
		const key = `${domain}:${skill}`;
		dispatch('run', { skill, domain, input: inputs[key] || undefined });
	}
</script>
						...
						{#each domain.skills as skill}
							<div class="rounded-sm p-2 space-y-1.5" style="background: var(--bg); box-shadow: 0 0 0 1px var(--ring-soft);">
								...
								<button
									...
									onclick={() => run(skill.slug, domain.slug)}
								>
									RUN
								</button>
							</div>
